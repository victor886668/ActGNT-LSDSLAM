...
def log_view(
    ...
    filename = os.path.join(out_folder, prefix[:-1] + "_{:03d}_coarse.png".format(save_index))
    ...
    if depth_coarse is not None:
        ...
        filename = os.path.join(
            out_folder, prefix[:-1] + "_{:03d}_coarse_depth.png".format(save_index)
        )
    ...
    if uncert_coarse is not None:
        ...
        filename = os.path.join(
            out_folder, prefix[:-1] + "_{:03d}_coarse_uncert.png".format(save_index)
        )
    ...
    if rgb_fine is not None:
        ...
        filename = os.path.join(out_folder, prefix[:-1] + "_{:03d}_fine.png".format(save_index))
        imageio.imwrite(filename, to8b(rgb_fine))

    if depth_fine is not None:
        depth_fine = depth_fine.permute(1, 2, 0).detach().cpu().numpy()
        filename = os.path.join(
            out_folder, prefix[:-1] + "_{:03d}_fine_depth.png".format(save_index)
        )
    ...
    if uncert_fine is not None:
        ...
        filename = os.path.join(
            out_folder, prefix[:-1] + "_{:03d}_fine_uncert.png".format(save_index)
        )
