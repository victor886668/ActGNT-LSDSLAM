...
# GNT(Generative Neural Transformer)网络的主要实现类
class GNT(nn.Module):
    def __init__(self, args, in_feat_ch=32, posenc_dim=3, viewenc_dim=3, ret_alpha=False):
        super(GNT, self).__init__()
        # RGB特征处理网络,将输入特征映射到网络宽度
        self.rgbfeat_fc = nn.Sequential(
            nn.Linear(in_feat_ch + 3, args.netwidth), # 输入特征通道数+3维RGB
            nn.ReLU(), # ReLU激活函数
            nn.Linear(args.netwidth, args.netwidth), # 映射到网络宽度
        )

        # 创建transformer模块列表
        # view_selftrans是射线transformer
        # view_crosstrans是视图transformer 
        self.view_selftrans = nn.ModuleList([])
        self.view_crosstrans = nn.ModuleList([])
        self.q_fcs = nn.ModuleList([]) # MLP列表
        
        # 根据transformer深度创建多层transformer
        for i in range(args.trans_depth):
            # 视图transformer
            view_trans = Transformer2D(
                dim=args.netwidth, # 特征维度
                ff_hid_dim=int(args.netwidth * 4), # 前馈网络隐藏层维度
                ff_dp_rate=0.1, # dropout率
                attn_dp_rate=0.1, # 注意力dropout率
            )
            ...
            if i % 2 == 0:
                ...
                q_fc = nn.Identity() # 使用恒等映射
            self.q_fcs.append(q_fc)

        # 保存位置编码和视角编码维度
        self.posenc_dim = posenc_dim 
        self.viewenc_dim = viewenc_dim
        self.ret_alpha = ret_alpha # 是否返回alpha值
        
        # 输出处理层
        self.norm = nn.LayerNorm(args.netwidth) # 层归一化
        self.rgb_fc = nn.Linear(args.netwidth, 3) # RGB输出层
        self.uncert_fc = nn.Linear(args.netwidth, 1) # 不确定性输出层
        self.relu = nn.ReLU()
        ...
        self.linear_fc = nn.Linear(args.netwidth, 1)
    ...
    def forward(self, rgb_feat, ray_diff, mask, pts, ray_d):
        ...
        viewdirs = self.view_enc(viewdirs) # 编码视角方向
        ...
        uncert = self.uncert_fc(h.mean(dim=1))
        raw = self.linear_fc(h.mean(dim=1))
        
        # 根据需要返回结果
        if self.ret_alpha:
            return torch.cat([outputs, attn], dim=1), uncert, raw
        else:
            return outputs, uncert, raw
