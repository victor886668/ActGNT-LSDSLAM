...
def train(args):
    ...
    holdout_datest = dataset_dict['nerf_nuscence_holdout'](args, scenes=args.eval_scenes)
    hold_loader = DataLoader(holdout_datest, batch_size=1)
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        if args.local_rank == 0:
            ...
            if epoch % args.active_iter == 0:
                ...
                if len(holdout_datest) == 0:
                    print("No holdout dataset!")
                elif len(holdout_datest) <= args.choose_k:
                    render_rgb_files, render_poses, render_intrinsics = holdout_datest.render_rgb_files, holdout_datest.render_poses, holdout_datest.render_intrinsics
                    ...
                    render_rgb_files, render_poses, render_intrinsics = holdout_datest.delete_indexs(indexs)
                    hold_loader = DataLoader(holdout_datest, batch_size=1)
                    ...
                    logstr = f"目前删除了{str(indexs)}个holdout数据，目前剩余{len(holdout_datest)}个holdout数据\n目前增加了{len(indexs)}个train数据，目前总共{len(train_dataset)}个train数据\n"
