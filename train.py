    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            if args.local_rank == 0:
                # 开始active部分
                if global_step in args.active_iter:
                    render_kwargs_train = {
                        "model": model,
                        "projector": projector,
                        "featmaps": featmaps,
                    ...
                    i_train = np.arange(0, 20)
                    i_holdout = np.arange(20, 100)
                    print('start evaluation:')
                    print('get rays')
                    rays = np.stack([get_rays_np(train_data["H"].cpu().numpy(), train_data["W"].cpu().numpy(), train_data["focal"].cpu().numpy(), p) for p in all_pose.cpu().numpy()[:, :3, :4]], 0)  # [N, ro+rd, H, W, 3]
                    print('done, concats')

                    # get all holdout rays (candidate)
                    images = train_dataset.images + holdout_dataset.images
