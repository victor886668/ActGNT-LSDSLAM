...
def save_poses_llff(basedir, poses, pts3d, perm):
    pts_arr = []
    vis_arr = []
    for k in pts3d:
        pts_arr.append(pts3d[k].xyz)
    ...
    for i in perm:
        ...
        # print( i, close_depth, inf_depth )
            ...
            save_arr_houldout.append(np.concatenate([poses[..., i].ravel(), np.array([close_depth, inf_depth])], 0))
    save_arr = np.array(save_arr)
    save_arr_houldout = np.array(save_arr_houldout)
    
    np.save(os.path.join(basedir, 'poses_bounds_train.npy'), save_arr)
    np.save(os.path.join(basedir, 'poses_bounds_houldout.npy'), save_arr)
...
def gen_poses(basedir, match_type, factors=None):
    ...
    #save_poses_llff(basedir, poses, pts3d, perm)
